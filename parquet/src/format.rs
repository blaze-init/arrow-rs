...
impl FileMetaData {
  pub fn new<F5, F6, F7, F8, F9, F10, F11>(version: i32, schema: Vec<SchemaElement>, num_rows: i64, row_groups: Vec<RowGroup>, key_value_metadata: F5, created_by: F6, column_orders: F7, encrypted: F8, keyname: F9, encryption_algorithm: F10, footer_signing_key_metadata: F11) -> FileMetaData where F5: Into<Option<Vec<KeyValue>>>, F6: Into<Option<String>>, F7: Into<Option<Vec<ColumnOrder>>>, F8: Into<Option<bool>>, F9: Into<Option<String>>, F10: Into<Option<EncryptionAlgorithm>>, F11: Into<Option<Vec<u8>>> {
    FileMetaData {
      version,
      schema,
      num_rows,
...
impl crate::thrift::TSerializable for FileMetaData {
  ...
  fn write_to_out_protocol<T: TOutputProtocol>(&self, o_prot: &mut T) -> thrift::Result<()> {
    ...
    if let Some(fld_var) = self.encrypted {
      o_prot.write_field_begin(&TFieldIdentifier::new("encrypted", TType::Bool, 8))?;
      o_prot.write_bool(fld_var)?;
      o_prot.write_field_end()?
    }
    if let Some(ref fld_var) = self.keyname {
      o_prot.write_field_begin(&TFieldIdentifier::new("keyname", TType::String, 9))?;
      o_prot.write_string(fld_var)?;
      o_prot.write_field_end()?
    }
