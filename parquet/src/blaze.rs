...
mod test {
    ...
    impl AsyncFileReader for AsyncLocalFileReader {
        fn get_bytes(&mut self, range: Range<u64>) -> BoxFuture<'_, crate::errors::Result<Bytes>> {
            Box::pin(async move {
                Ok(self.0.get_bytes(range.start, (range.end - range.start) as usize)?)
            })
        }
    ...
    async fn test_get_dictionary() -> crate::errors::Result<()> {
        ...
        let dict = get_dictionary_for_pruning(&mut builder.input.0, builder.metadata.row_group(0), col_idx)
            .unwrap()
