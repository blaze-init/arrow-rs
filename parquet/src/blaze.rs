...
pub fn get_dictionary_for_pruning<T: AsyncFileReader + Send + 'static>(
    input: &mut T,
    ...
    {
        if column_metadata.encodings().iter().any(|enc| {
            !matches!(
                enc,
        ...
        ($r:expr, $parquet_type:ty, $array_type:ty) => {{
            match $r.page_reader.get_next_page()? {
                Some(Page::DictionaryPage {
                    ...
                    Ok(Some(Arc::new(<$array_type>::from_iter_values(dict_values))))
                }
                _ => Ok(None),
            }
        }};
    ...
    let unsafe_page_reader = unsafe { std::mem::transmute::<_, Box<dyn PageReader>>(page_reader) };
    match get_column_reader(column_metadata.column_descr_ptr(), unsafe_page_reader) {
        ...
        ColumnReader::ByteArrayColumnReader(mut r) => match r.page_reader.get_next_page()? {
            Some(Page::DictionaryPage {
                buf, num_values, ..
            }) => {
...
impl<T: AsyncFileReader + Send + 'static> ChunkReader for WrappedChunkReader<'_, T> {
    ...
    fn get_bytes(&self, start: u64, length: usize) -> crate::errors::Result<Bytes> {
        ...
        Ok(inner.get_bytes_sync(start..end)?)
    }
}
...
mod test {
    ...
    impl AsyncFileReader for AsyncLocalFileReader {
        fn get_bytes(&mut self, range: Range<u64>) -> BoxFuture<'_, crate::errors::Result<Bytes>> {
            Box::pin(async move {
                Ok(self
                    .0
                    .get_bytes(range.start, (range.end - range.start) as usize)?)
            })
        }
        ...
        fn get_metadata<'a>(
            ...
            Box::pin(async move {
                ...
                self.0
                    .seek(SeekFrom::End(-(FOOTER_SIZE as i64) - metadata_len as i64))?;

                let mut buf = Vec::with_capacity(metadata_len);
                self.0
                    .try_clone()?
                    .take(metadata_len as _)
                    .read_to_end(&mut buf)?;
    ...
    async fn test_get_dictionary() -> crate::errors::Result<()> {
        let input = AsyncLocalFileReader(File::open(
            "/Volumes/Workspace/blaze-init/arrow-rs/with-dict-2.parquet",
        )?);
        ...
        let dict = get_dictionary_for_pruning(
            &mut builder.input.0,
            builder.metadata.row_group(0),
            col_idx,
        )
        .unwrap()
        ...
        eprintln!(
            "dict bytes: {}",
            dict_strs.offsets()[dict_strs.len()] - dict_strs.offsets()[0]
        );
